...
public class CartRestController {
    ...
    public PagedModel<EntityModel<CartItemReadDto>> getCartByUserId(@AuthenticationPrincipal UserDetails userDetails,
                                                                    @PageableDefault(sort = "id", size = 15) Pageable pageable,
                                                                    PagedResourcesAssembler<CartItemReadDto> assembler) {
        return assembler.toModel(cartService.findCartByUserId(userDetails.getUsername(), pageable));
    }
